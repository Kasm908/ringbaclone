...
def assert_url_is_public(url: str) -> str:
    """
    SSRF guard: reject URLs that target internal infrastructure before we
    ever fetch them. Blocks non-http(s) schemes and any host that resolves
    to a loopback, private, link-local (incl. cloud metadata 169.254.169.254),
    or otherwise-reserved address.
    """
    parsed = urlparse(url)
    if parsed.scheme not in ("http", "https"):
        raise UnsafeURLError(f"Blocked non-http(s) URL scheme: {parsed.scheme!r}")
    ...
    for info in infos:
        ...
        if (ip.is_private or ip.is_loopback or ip.is_link_local
            ...
            raise UnsafeURLError(f"Blocked internal address {ip} for host {host!r}")
    return url
...
def extract_campaign_data(url: str) -> dict:
    try:
        ...
        logger.warning(f"SSRF blocked in extract_campaign_data: {e}")
        return {}
...
def extract_phone_from_url(url: str) -> str:
    ...
    # SSRF guard: never let the scraper fetch an internal address.
    try:
        assert_url_is_public(url)
    except UnsafeURLError as e:
        logger.warning(f"SSRF blocked in extract_phone_from_url: {e}")
        return ""
    ...
    try:
        ...
        try:
            ...
            # SECURITY: the URL is passed via argv (sys.argv[1]) and never
            # interpolated into the executed source, so a crafted URL cannot
            # break out of the string literal and run arbitrary code.
            _SHOT_SCRIPT = (
                ...
                "    p = b.new_page()\n"
                "    p.goto(target, timeout=30000, wait_until=\"networkidle\")\n"
                "    p.wait_for_timeout(5000)\n"
                "    sys.stdout.buffer.write(p.screenshot(full_page=True))\n"
                "    b.close()\n"
            )
            screenshot_result = subprocess.run(
                [sys.executable, "-c", _SHOT_SCRIPT, url],
                capture_output=True,
                timeout=60,
            )
